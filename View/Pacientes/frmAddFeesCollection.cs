        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ConnectionNode.ExecuteSQLQuery(" INSERT INTO StudentLedger (STUDENT_ID, FEES_ID, EntryDate, EntryTime, User_ID, Debit, Credit, Narration, Month, PayMode) VALUES " +
                    " (" + txtStuID.Text + ", 0, '" + Strings.Format(dtpEntryDate.EditValue, "MM/dd/yyyy") + ("', '" + DateTime.Now.ToShortTimeString() + "', ") + Convert.ToString(ConnectionNode.userID) + ",0, " + Convert.ToString(UtilitiesFunctions.str_repl(txtAmount.Text)) + ",   '" + Convert.ToString(UtilitiesFunctions.str_repl(txtNarration.Text)) + ("', '" + cmbMonth.Text + "', '" + cmbPayMode.Text + "') "));
                UtilitiesFunctions.Logger(ConnectionNode.userID, DateTime.Now.ToShortTimeString(), "School payment, ID # " + txtSearch.Text + " , amount # " + txtAmount.Text);
